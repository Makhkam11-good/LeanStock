  ...
  return (
    <PageShell title="Stock inventory" description="Inventory list, filters, lot detail, reservation and release flows from `/inventory`.">
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
          ...
          <div className="mt-4">
            <DataTable
              ...
              columns={[
                ...
                { header: "Warehouse", cell: (item) => item.location?.warehouse?.name ?? "-" },
                ...
                { header: "Reorder", cell: (item) => item.reorder_point },
                {
                  header: "Detail",
                  cell: (item) => (
                    <Button variant="secondary" className="px-3" title="View lots" onClick={() => setSelectedId(item.id)}>
        ...
        <Card>
          <SectionHeader title="Inventory detail" description="Lot-level data is available from `/inventory/:id`." />
          {!selectedId ? (
            <p className="text-sm text-slate-500">Select an inventory row to inspect lots and reservations.</p>
          ) : selected.isLoading ? (
            <LoadingState />
          ) : selected.error ? (
